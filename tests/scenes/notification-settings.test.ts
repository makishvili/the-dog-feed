...
import { notificationSettingsScene, setGlobalDatabaseForNotificationSettings } from '../../src/scenes/notification-settings';
...
describe('notificationSettingsScene', () => {
  ...
  describe('hears "🔔 Включить уведомления"', () => {
    it('should show error when database is not initialized', async () => {
      ...
      const handler = hearsHandlers.find((h: any) => h.triggers.includes('🔔 Включить уведомления'));
    ...
    it('should enable notifications and reenter scene', async () => {
      ...
      mockDatabase.updateUserNotifications = jest.fn().mockResolvedValue(undefined);
      ...
      expect(mockDatabase.updateUserNotifications).toHaveBeenCalledWith(1, true);
    ...
    it('should show error message when database error occurs during update', async () => {
      ...
      mockDatabase.updateUserNotifications = jest.fn().mockRejectedValueOnce(new Error('Database error'));
      ...
      expect(ctx.reply).toHaveBeenCalledWith('❌ Ошибка сохранения настроек');
    });
  });
  ...
  describe('hears "🔕 Выключить уведомления"', () => {
    it('should show error when database is not initialized', async () => {
      ...
      const hearsHandlers = (notificationSettingsScene as any).hearsHandlers;
      // Находим нужный обработчик по паттерну
      const handler = hearsHandlers.find((h: any) => h.triggers.includes('🔕 Выключить уведомления'));
      await handler.handler(ctx);
    ...
    it('should disable notifications and reenter scene', async () => {
      ...
      expect(mockDatabase.updateUserNotifications).toHaveBeenCalledWith(1, false);
      expect(ctx.scene.reenter).toHaveBeenCalled();
    });
  ...
  describe('on text (unknown command)', () => {
    it('should show menu and prompt to use buttons when database is not initialized', async () => {
      ...
      await (notificationSettingsScene as any).onMiddleware('text')[0](ctx);
      
      expect(ctx.reply).toHaveBeenCalledWith('Используйте кнопки меню для навигации.');
    });
