...
describe('ExportService', () => {
  ...
  describe('exportFeedings', () => {
    it('should throw error if no data to export', async () => {
      mockDatabase.getFeedingsForPeriod = jest.fn().mockResolvedValueOnce([]);

      const options = { format: 'csv' as const };

      await expect(exportService.exportFeedings(options)).rejects.toThrow('Нет данных для экспорта');
    });
    ...
    it('should export data in CSV format', async () => {
      ...
      mockDatabase.getFeedingsForPeriod = jest.fn().mockResolvedValueOnce(mockFeedings);
  ...
  describe('getFeedingsForExport', () => {
    it('should get feedings for week period', async () => {
      ...
      const result = await (exportService as any).getFeedingsForExport(options);
    ...
    it('should get feedings for month period', async () => {
      ...
      // Вызовем приватный метод getFeedingsForExport через any
      const options = { format: 'csv' as const, period: 'month' as const };
      const result = await (exportService as any).getFeedingsForExport(options);
  ...
  describe('generateCSV', () => {
    it('should generate correct CSV content', () => {
      ...
      expect(csvContent).toContain('Дата,Время,Пользователь,Тип корма,Количество (г),Детали');
  ...
  describe('generateHTML', () => {
    it('should generate correct HTML content', () => {
      ...
      expect(htmlContent).toContain('<title>История кормлений собаки</title>');
      expect(htmlContent).toContain('📊 Статистика');
      expect(htmlContent).toContain('<strong>Всего кормлений:</strong> 1');
      expect(htmlContent).toContain('<strong>Влажный корм:</strong> 1 раз');
