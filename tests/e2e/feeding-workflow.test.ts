...
describe('E2E Feeding Workflow Tests', () => {
  ...
  describe('Feeding Workflow', () => {
    ...
    it('should stop feedings and notify users', async () => {
      ...
      const otherActionsHandler = (mainScene as any).hears(/Другие действия/);
      ...
      const stopFeedingsHandler = (otherActionsScene as any).hears(/⏹️ Завершить кормления на сегодня/);
      ...
      expect(ctx.telegram.sendMessage).toHaveBeenCalledWith(123456789, expect.stringContaining('⏹️ Кормления приостановлены.'));
    });
  });
  ...
  describe('Scheduled Feeding Workflow', () => {
    it('should schedule feeding and cancel it', async () => {
      ...
      const scheduleFeedingHandler = (otherActionsScene as any).hears(/📅 Запланировать кормление/);
      ...
      const viewScheduledHandler = (otherActionsScene as any).hears(/📋 Просмотреть запланированные/);
      ...
      mockSchedulerService.cancelScheduledFeeding.mockResolvedValueOnce(undefined);
      
      const cancelFeedingHandler = (scheduledListScene as any).hears(/❌ Отменить кормление/);
    ...
    it('should schedule feeding and cancel all feedings', async () => {
      ...
      // Нажатие на кнопку "📋 Просмотреть запланированные"
      const viewScheduledHandler2 = (otherActionsScene as any).hears(/📋 Просмотреть запланированные/);
      ...
      mockSchedulerService.cancelAllScheduledFeedings.mockResolvedValueOnce(1);
      
      const cancelAllFeedingsHandler = (scheduledListScene as any).hears(/❌ Отменить все/);
