...
export class DatabaseService {
  ...
  private async runMigrations(): Promise<void> {
    try {
      ...
      const checkDetailsColumn = await this.checkColumnExists('feedings', 'details');
  ...
  private checkColumnExists(tableName: string, columnName: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
  ...
  private addColumn(tableName: string, columnName: string, columnType: string): Promise<void> {
    return new Promise((resolve, reject) => {
    ...
    try {
      ...
      for (const setting of defaultSettings) {
        ...
        if (existingSetting === null) {
          ...
          console.log(`Установлена настройка по умолчанию: ${setting.key} = ${setting.value}`);
        }
      }
    } catch (error) {
  ...
  async updateUserInterval(telegramId: number, intervalMinutes: number): Promise<void> {
    return new Promise((resolve, reject) => {
      this.db.run(`
        UPDATE users SET feeding_interval = ? WHERE telegram_id = ?
      `, [intervalMinutes, telegramId], (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
  ...
  async updateUserNotifications(userId: number, enabled: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      this.db.run(`
        UPDATE users SET notifications_enabled = ? WHERE id = ?
      `, [enabled ? 1 : 0, userId], (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
  ...
  async getStats(): Promise<{ totalFeedings: number; todayFeedings: number; totalUsers: number }> {
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        let totalFeedings = 0;
        let todayFeedings = 0;
        ...
        this.db.get('SELECT COUNT(*) as count FROM feedings', (err, row: any) => {
          if (err) reject(err);
          else {
  ...
  async getFeedingsForPeriod(startDate?: Date, endDate?: Date, limit?: number): Promise<DatabaseFeeding[]> {
    return new Promise((resolve, reject) => {
      let query = `
        SELECT id, user_id, timestamp, food_type, amount, details
        FROM feedings 
  ...
  async getFeedingsWithPagination(page: number = 1, limit: number = 10): Promise<DatabaseFeeding[]> {
    return new Promise((resolve, reject) => {
  ...
  async getTotalFeedingsCount(startDate?: Date, endDate?: Date): Promise<number> {
    return new Promise((resolve, reject) => {
  ...
  async getScheduledFeedingById(scheduleId: number): Promise<DatabaseScheduledFeeding | null> {
    return new Promise((resolve, reject) => {
      this.db.get(`
        SELECT id, scheduled_time, is_active, created_by, created_at
        FROM scheduled_feedings 
        WHERE id = ?
      `, [scheduleId], (err, row: any) => {
        if (err) {
          reject(err);
        } else if (row) {
          resolve({
