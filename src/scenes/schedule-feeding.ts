...
export function setGlobalSchedulerForScheduleFeeding(
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('будущем')) {
                ...
                errorMessage =
                    '❌ Максимум 10 запланированных кормлений одновременно';
...
scheduleFeedingScene.hears(/❌ Отменить ввод/, ctx => {
    ctx.reply('❌ Создание кормления отменено.', getScheduleFeedingKeyboard());
    ctx.scene.enter(SCENES.MAIN);
});
